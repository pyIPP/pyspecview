...
class loader_FIRETIP(loader):
    ...
    def __init__(self,*args, **kargs):
        ...
        super(loader_FIRETIP,self).__init__(*args, **kargs)
        ...
        #import IPython
        #IPython.embed()
        ##plot(t,OMFIT['MICROWAVE']['FIRETIP']['ANALYSIS']['DENSITY'].data())
        #self.MDSconn.get( 'getnci("\\MICROWAVE::TOP.FIRETIP.ANALYSIS:DENSITY","fullpath")' )
        
        #self.MDSconn.get(f'getnci("{raw_path}:*:INPUT_01", "fullpath")').data()
        ...
        self.names = 'CH1', 'CH2', 'CH3', 'CH7'
        self.MDSconn.openTree(self.tree,self.shot)
        self.rtan = [self.MDSconn.get("\\MICROWAVE::TOP.FIRETIP.ANALYSIS:RTAN_"+n[0]+n[2]).data() for n in self.names] 
    ...
    def get_signal(self,group, names,calib=False,tmin=None,tmax=None):
        ...
        #print(name)
        if name != 'CH2':
            node = node+name[0]+name[2]
            
        if name not in self.data:
            self.MDSconn.openTree(self.tree,self.shot)

            self.data[name] = self.MDSconn.get( "\\MICROWAVE::TOP.FIRETIP.ANALYSIS:"+node).data()
            #print(self.data[name].dtype)

        
            if not hasattr(self,'tvec'):
                #BUG how to get the right time vector?? 
                self.tvec = arange(len(self.data[name]))/50e4
        ...
        return self.tvec[ind], self.data[name][ind]
    ...
    def signal_info(self,group,name,time):
        ...
        Rtg = self.rtan[self.names.index(name)]
        #print(Rtg, type(Rtg))
 
        rho = self.eqm.rz2rho(float(Rtg)/100,0,time,coord_out=self.rho_lbl)[0]

        info = f'{group}:{name}, Rtag = {Rtg}cm,  rho_tg = %.2f'%rho
        return info
...
def main():
    ...
    sxr = loader_FIRETIP(121009,exp='NSTX',eqm=eqm,rho_lbl='rho_pol',MDSconn=MDSconn)
    data1 = sxr.get_signal( 'DEN',('CH2',),tmin=-infty, tmax=infty,calib=True)
