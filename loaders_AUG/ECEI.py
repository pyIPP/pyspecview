...
class loader_ECEI(loader):
    ...
    def __init__(self,*args, **kargs):
        ...
        if self.dd.Open('RZO', self.shot, experiment="ECEI", edition=0):
            ...
            self.R_old = self.dd.GetSignalGroup('R')
            self.z_old = self.dd.GetSignalGroup('z')  
            self.rho_old = self.dd.GetSignalGroup('rho')  

        if self.dd.Open('RZN', self.shot, experiment="ECEI", edition=0):
            self.RZtime_new = self.dd.GetTimebase('time')
            self.R_new = self.dd.GetSignalGroup('R')
            self.z_new = self.dd.GetSignalGroup('z')  
            self.rho_new = self.dd.GetSignalGroup('rho')  
            self.dd.Close()
    ...
    def get_signal(self,group, name,calib=False, tmin=None,tmax=None):
        ...
        if group == 'ECI':
            ...
            if not hasattr(self, 'TDI_%d'%sig):
                ...
                data = self.dd.GetSignalGroup("Sig%d"%sig)
    ...
    def get_rho(self,group,names,time,dR=0,dZ=0):
        ...
        return rho_p, arctan2(z-z0, R-r0), R,z, #,R_, z_
