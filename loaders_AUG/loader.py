...
class loader(object):
    ...
    def mag_theta_star(self,tshot = 4 ,rho=0,dR=0,dZ=0,n_theta = 100):
        ...
        r0 = np.interp(tshot, self.eqm.t_eq, self.eqm.ssq['Rmag'])+dR
        z0 = np.interp(tshot, self.eqm.t_eq, self.eqm.ssq['Zmag'])+dZ
...
# GIT
        jt = np.argmin(np.abs(self.eqm.t_eq - time))
        nrho = np.max(self.eqm.lpfp) + 1
        psi = self.eqm.get_profile('PFL')[jt, :nrho]
        q   = self.eqm.get_profile('Qpsi')[jt, :nrho]
        rho_q = self.eqm.rho2rho(psi, t_in=time, coord_in='Psi', coord_out=self.rho_lbl)[0]
        #find outermost position of the resonance surface
        rho_surf = np.interp( qvalues, np.abs(q), rho_q, left=np.nan)
