...
class loader(object):
    ...
    def get_plasma_freq(self,rho):
        ...
        if not  hasattr(self,'plasma_freq'):
            ...
            names = self.dd.getSignalNames()
            if 'R_time' in names:
                R = cer.GetAreabase('R_time')
                z = cer.GetAreabase('z_time') 
            else:
                R = cer.GetAreabase('R')[None]
                z = cer.GetAreabase('z')[None]

            ind = R.mean(0)!= 0
            R = R[:,ind]
            z = z[:,ind]

            V = cer.GetSignalGroup('vrot')[:,ind]
            T = cer.GetTimebase('vrot')
