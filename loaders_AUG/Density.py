...
class loader_DCN(loader):
    ...
    def unwrap_ne(self,tvec, sig,ref):
        ...
        pos = loc_max(sig,N)
        max_sig = np.interp(arange(len(tvec)),pos, sig[pos])

        pos = loc_max(-sig,N)
        min_sig = np.interp(arange(len(tvec)),pos, sig[pos])
        

        pos = loc_max(ref,N)

        max_ref = np.interp(arange(len(tvec)),pos, ref[pos])

        pos = loc_max(-ref,N)

        min_ref = np.interp(arange(len(tvec)),pos, ref[pos])
        ...
        knots,vals = [],[]
        ind_knots = np.r_[list(range(0,len(tvec),N*1000)),-1]
        peaks = np.infty
        ind = slice(None,None)
    ...
    def get_signal(self,group, name,calib=False,tmin=None,tmax=None):
        ...
        if name[0] == 'H':  #DCN lasers
            ...
            phi2  = unwrap(np.arctan2(cos_2,sin_2)) #this signal has stronger mode signal
