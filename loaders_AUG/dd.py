...
class shotfile:
   ...
   def Open(self, diagname, shotnumber, experiment='AUGD', edition=0):
      ...
      _shot     = ct.byref(shot)
      diag      = ct.c_char_p(diagname.encode())
      self.diagname = diagname
      exp       = ct.c_char_p(experiment.encode())
      dat       = 18*b'd'
...
# 3 dims, as the 0-sized are removed. Usually (always?) it has 1 dim.
   ...
   def GetParameterSetInfo( self , name ):
      ...
      if self.__status:
         ...
         _error   = ct.byref( error )
         _diaref  = ct.byref( self.__diaref )
         par_name = ct.c_char_p(name.encode())
         ...
         output.devsig = []
         for i in range(info):
            tmp = rname.value[8*i:8*(i+1)].decode('utf-8')
            if tmp.strip() != '':
   ...
   def GetParameterInfo(self, set_name, par_name):
      ...
      if self.__status:
         ...
         _error  = ct.byref(error)
         _diaref = ct.byref(self.__diaref)
         pset    = ct.c_char_p(set_name.encode())
         par     = ct.c_char_p(par_name.encode())
         item    = ct.c_uint32(0)
         _item   = ct.byref(item)
   ...
   def GetParameter(self, set_name, par_name):
      ...
      if self.__status:
         ...
         if info.error == 0:
            ...
            _error = ct.byref(error)
            _diaref = ct.byref(self.__diaref)
            setn = ct.c_char_p(set_name.encode())
            lset = ct.c_ulonglong(len(set_name))
            par = ct.c_char_p(par_name.encode())
            lpar = ct.c_ulonglong(len(par_name))
   ...
   def GetSignal(self, signame, cal=False,nbeg=0,nend=np.infty):
      ...
      if self.__status:
         ...
         if info.error == 0:
            ...
            _lbuf    = ct.byref(lbuf)
            signam   = ct.c_char_p(signame.encode())
            physdim  = 8*b'p'
...
# Calibrated SignalGroup
                  ...
                  if error.value != 0:
                     ...
                     self.units = _physdim.value.decode('utf-8')
               else:
   ...
   def GetAreabase(self, signame,nbeg=1,nend=np.infty):
      ...
      if self.__status:
         ...
         if info.error == 0:
            ...
            _error  = ct.byref(error)
            _diaref = ct.byref(self.__diaref)
            name    = ct.c_char_p(signame.encode())
   ...
   def GetObjectValue(self, name, field):
      ...
      if self.__status:
         ...
         _diaref = ct.byref(self.__diaref)
         _name   = ct.c_char_p(name.encode())
         _field  = ct.c_char_p(field.encode())
         lname   = ct.c_ulonglong(len(name))
   ...
   def GetObjectData(self, obj_name):
      ...
      if self.__status:
         ...
         _error  = ct.byref(error)
         _diaref = ct.byref(self.__diaref)
         name    = ct.c_char_p(obj_name.encode())
   ...
   def GetObjectHeader(self, name):
      if self.__status:
         ...
         if error.value == 0:
            ...
            output.text   = _text.value.decode('utf-8')
         return output
      return None
   ...
   def GetObjectName(self, obj):
      if self.__status:
         ...
         if error.value == 0:
            return _name.value.decode('utf-8').replace(' ','')
      return -1
