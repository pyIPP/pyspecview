    ...
    def on_click(self,event):
        ...
        if event.button is 2:
            ...
            if not self.zoomed:
                ...
                # Hide all the other axes...
                    if axis is not ax:
                        #make them invisible and minimize
                        axis.set_visible(False)
                        axis.set_position([0,0.01,0.01,0.01])

            # On right click, restore the axes
            else:
                #self.zoomed_axis.set_position(self.zoomed_axis._orig_position)
                self.zoomed = False
                for axis in event.canvas.figure.axes:
...
#    
        ...
        if line == '\n':
            if len(r) > 0:
                if pol:
                    ...
                    vessel.append({'r':np.array(r)*np.cos(np.deg2rad(-22.5*3.))-np.array(z)*np.sin(np.deg2rad(-22.5*3.)),
                                   'z':np.array(r)*np.sin(np.deg2rad(-22.5*3.))+np.array(z)*np.cos(np.deg2rad(-22.5*3.)) })
            r=[]
            z=[]
            continue
        val = line.split()
    ...
    for key in range(len(vessel)):
        if polygon:
            ax.add_patch(Polygon(list(zip(vessel[key]['r'],vessel[key]['z'])),facecolor='grey',edgecolor='none'))
        else:        
            ax.plot(vessel[key]['r'],vessel[key]['z'],'k-')
        


def plt_eq_pol(ax,nshot,time,rhos=np.linspace(0,1,10),linewidth=1,linecolor='k',linestyleSep='solid',linestyleFlux='dashed',diag='EQI',exp='AUGD',ed=0):
    import kk
    kk = kk.KK()
...
def legendThickLines(leg,width=10):
